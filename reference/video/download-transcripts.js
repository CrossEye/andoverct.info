...
function transcriptParagraphs(meeting, vttText) {
  ...
  // speaker markers, regardless of where the video is hosted.
  const isDeepgram = meeting.transcriber === "deepgram";

  // Parse and process cues based on type
  let cues;
  if (isZoom || isDeepgram) {
...
function convertVttToHtml(meeting, vttText) {
  ...
  const disclaimerSource = isDeepgram
    ? "This transcript was generated by Deepgram from the meeting recording."
    : isZoom
...
function generateIndex(meetings, transcriptFiles) {
  ...
  if (hasZoom && hasYouTube) {
    disclaimerText = "Transcripts were auto-generated by YouTube, OpenAI Whisper, or Deepgram.";
  } else if (hasZoom) {

...
// Ticks from the first multiple of `step` at or above y0, up to y1. A numeric
// sweep with a computed stride, so it is generated rather than walked.
const ticksFrom = (y0, y1, step) => {
  const first = Math.ceil(y0 / step) * step
  const count = Math.max(0, Math.floor((y1 - first) / step) + 1)
  return Array.from({ length: count }, (_, i) => first + i * step)
}

// Push each label down far enough to clear the one above it. A running fold:
// every position depends on the adjusted position before it, not the original.
const spread = (values, minGap) => values.reduce((acc, v) => {
...
function decomp(ctx, groupId, args = {}) {
  ...
  pts(en).forEach((p) => out.push(el('rect',
    { x: p[0] - 3.4, y: p[1] - 3.4, width: 6.8, height: 6.8,
      fill: PAL.slate, stroke: PAL.markerEdge, 'stroke-width': 1.1 })))
  out.push(line(pp, PAL.green, 4.2))
  pts(pp).forEach((p) => out.push(el('circle',
    { cx: p[0], cy: p[1], r: 4.6, fill: PAL.green, stroke: PAL.markerEdge, 'stroke-width': 1.2 })))
  ...
  rows.filter((r, i) => showYear(i)).forEach((r) => {
    out.push(text(X(r.fy), FH - M.b + 22, 'FY' + r.fy,
      { 'text-anchor': 'middle', 'font-size': 11.5, fill: PAL.axis }))
  })
  ...
  const ys = spread(labels.map((l) => Y(l.v)), 20)

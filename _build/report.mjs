...
const normalizeForNumeric = (text) => {
  let prev = null
  while (prev !== text) {
    prev = text
...
const sectionLabelFor = (headingText, sectionLabels) => {
...
const addSectionLabels = (htmlBody, sectionLabels) => {
...
const addPartBreakClass = (htmlBody) => {
...
const keepIntroWithTable = (htmlBody) => {
  return htmlBody.replace(
    /<p>([^<]*(?:<(?!\/?p>)[^<]*)*)<\/p>(\s*)(<table)/gs,
...
const rewriteRow = (rowHtml, cellTag, numericCols) => {
...
const alignNumericColumns = (htmlBody) => {
...
const restoreTypography = (htmlBody) => {
...
const replaceOtherFormatsSection = (htmlBody, formats) => {
...
const addHeaderAnchors = (htmlBody) => {
  const used = new Set()
...
const breadcrumbHtml = (meta, sections) => {
  const trail = [
    { label: "Home", href: "/" },
...
const privateBreadcrumbHtml = (meta) => {
  return (
...
const buildHtml = async (mdText, meta, { forPdf, extraCss, plugin, themeCss, breadcrumb, isPrivate, asOf }) => {
...
const validateMeta = (meta, mdPath) => {
...
const isPrivateReportMd = (mdPath) => {
  try {
    const { data } = matter(readFileSync(mdPath, "utf8"))
...
const preprocessDirectives = (mdBody, folder, meta, correspondence) => {
...
const buildReport = async (mdPath) => {
